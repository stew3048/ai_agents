"""
DINO-MLP 訓練腳本（Colab 專屬版本，參數化）

訓練 DINO → DL 方案：SegFormer-like Decoder（MLP-based）
所有路徑和參數都可通過 CLI 指定
DINO: 凍結（frozen）

輸出：
- {outputs_dir}/train_dino_mlp_YYYYMMDD_HHMMSS/checkpoints/best.pth
- {outputs_dir}/train_dino_mlp_YYYYMMDD_HHMMSS/training_log.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description='訓練 DINO-MLP（Colab 專屬版本，參數化）')
    ...
    print("=" * 60)
    print("  訓練 DINO → DL：SegFormer-like Decoder（MLP-based）")
    ...
    output_dir = os.path.join(args.outputs_dir, f"train_dino_mlp_{timestamp}")

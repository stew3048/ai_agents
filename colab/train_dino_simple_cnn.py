"""
DINO-SimpleCNN 訓練腳本（Colab 專屬版本，參數化）

訓練 DINO → DL 方案：簡單 CNN Decoder
所有路徑和參數都可通過 CLI 指定
DINO: 凍結（frozen）

輸出：
- {outputs_dir}/train_dino_simple_cnn_YYYYMMDD_HHMMSS/checkpoints/best.pth
- {outputs_dir}/train_dino_simple_cnn_YYYYMMDD_HHMMSS/training_log.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description='訓練 DINO-SimpleCNN（Colab 專屬版本，參數化）')
    ...
    print("=" * 60)
    print("  訓練 DINO → DL：簡單 CNN Decoder")
    ...
    output_dir = os.path.join(args.outputs_dir, f"train_dino_simple_cnn_{timestamp}")

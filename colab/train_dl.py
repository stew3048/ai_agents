"""
DL baseline 訓練腳本（Colab 專屬版本，參數化）

使用 in-domain split（train_list.txt, val_list.txt）訓練 DL baseline (U-Net)
所有路徑和參數都可通過 CLI 指定

輸出：
- {outputs_dir}/train_dl_YYYYMMDD_HHMMSS/checkpoints/best.pth
- {outputs_dir}/train_dl_YYYYMMDD_HHMMSS/training_log.csv
"""
...
def main():
    parser = argparse.ArgumentParser(description='訓練 DL baseline（Colab 專屬版本，參數化）')
    ...
    print("=" * 60)
    print("  訓練 DL baseline（Colab 專屬版本，參數化）")
    print("=" * 60)
    print()
    print(f"環境: {env}")
    print(f"設備: {device}")
    ...
    output_dir = os.path.join(args.outputs_dir, f"train_dl_{timestamp}")

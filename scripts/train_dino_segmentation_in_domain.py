"""
使用 in-domain split 訓練 DINOv2SegmentationModel（多層特徵 + 多層卷積解碼器）

- 資料：outputs/train_list.txt（2535 筆）、outputs/val_list.txt（315 筆）
- 模型：models/dinov2_segmentation_decoder.py（Backbone 凍結，只訓練解碼器）
- Loss：BCEWithLogitsLoss + Dice（CombinedLoss），輸出 logits
- 參數：batch_size=2, epochs=10, lr=1e-4, image_size=(160,160)

輸出：
- outputs/train_dino_segmentation_YYYYMMDD_HHMMSS/checkpoints/best.pth
- outputs/train_dino_segmentation_YYYYMMDD_HHMMSS/training_log.csv
"""
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='訓練 DINOv2 多層卷積解碼器（in-domain split）')
    parser.add_argument('--device', type=str, default=None, help='cpu / cuda，預設自動偵測')
    args = parser.parse_args()

    print("=" * 60)
    print("  DINOv2 多層卷積解碼器 in-domain 訓練")
    print("=" * 60)
    print()
    ...
    print("建立模型: DINOv2SegmentationModel（最後四層 concat 1536 → 4-block 卷積解碼器）")
    ...
    output_dir = os.path.join(_root, "outputs", f"train_dino_segmentation_{timestamp}")

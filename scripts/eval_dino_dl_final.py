"""
評估 DINO → DL pipeline（四種方案，使用固定 test_list.txt）

輸入：
- outputs/test_list.txt（固定，每行一個相對路徑）
- outputs/train_dino_mlp_*/checkpoints/best.pth
- outputs/train_dino_simple_cnn_*/checkpoints/best.pth
- outputs/train_dino_fpn_*/checkpoints/best.pth
- outputs/train_dino_hybrid_*/checkpoints/best.pth

處理：
- 讀取 test_list.txt 的每張圖（相對路徑需加上 data/ 前綴載入）
- 用四種 DINO → DL checkpoint 推論
- 計算 IoU / FP / FN（與 GT mask 對齊）
- Per-camera 統計

輸出：
- 追加到 outputs/metrics_summary.csv（method='DINO-MLP', 'DINO-CNN', 'DINO-FPN', 'DINO-Hybrid'）
"""
...
def find_latest_checkpoint(pattern):
    ...
    candidates = list(outputs_dir.glob(f'{pattern}*/checkpoints/best.pth'))
    if not candidates:
        return None
    return str(max(candidates, key=os.path.getmtime))


def load_dino_model(model_type, checkpoint_path, device):
    """
    載入 DINO → DL 模型
    
    參數:
        model_type: 'mlp', 'simple_cnn', 'fpn', 'hybrid'
        checkpoint_path: checkpoint 路徑
        device: 設備
    """
    if model_type == 'mlp':
        model = DINOWithMLPDecoder(device=device, target_size=256)
    elif model_type == 'simple_cnn':
        model = DINOWithSimpleCNNDecoder(device=device, target_size=256)
    elif model_type == 'fpn':
        model = DINOWithFPNDecoder(device=device, target_size=256)
...
class NormalizedDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # DINO 需要 normalize（但 hybrid 方案模型內部會處理）
...
def evaluate_per_image_dino(model, dataloader, device, use_amp=False, model_type='mlp', overlay_dir=None, image_size=(256, 256)):
    """
    評估每張影像（DINO → DL）
    ...
    with torch.no_grad():
        for batch_idx, (images, masks) in enumerate(tqdm(dataloader, desc=f'  Evaluating DINO-{model_type.upper()}', leave=False)):
            ...
            for i in range(images.shape[0]):
                ...
                if overlay_dir:
                    try:
                        ...
                        overlay_filename = f"{camera_id}_{image_base}_dino_{model_type}_overlay.png"
...
def evaluate_one_model(model_type, checkpoint_path, test_list, device, use_amp, image_size, overlay_dir=None):
    """
    評估單一 DINO → DL 模型
    
    返回:
        Dict[camera_id, metrics]: per-camera metrics
    """
    print(f"\n評估 DINO → DL 方案：{model_type.upper()}")
    print(f"  Checkpoint: {checkpoint_path}")
    
    # 載入模型
    model = load_dino_model(model_type, checkpoint_path, device)
    ...
    # 包裝成 normalized dataset（hybrid 方案例外，它需要原始影像）
    if model_type == 'hybrid':
        # Hybrid 方案需要原始影像（未 normalize），模型內部會處理
    ...
    # 評估
    results = evaluate_per_image_dino(model, dataloader, device, use_amp, model_type, overlay_dir=overlay_dir, image_size=image_size)
...
def main():
    parser = argparse.ArgumentParser(description='評估 DINO → DL（四種方案，使用固定 test_list.txt）')
    ...
    parser.add_argument('--output_csv', type=str, default='outputs/metrics_summary.csv',
                        help='輸出 CSV 路徑')
    parser.add_argument('--overlay_dir', type=str, default='outputs/dino_dl_overlays',
                        help='overlay 輸出目錄（預設：outputs/dino_dl_overlays）')
    ...
    print("=" * 60)
    print("  評估 DINO → DL pipeline（四種方案，使用固定 test_list.txt）")
    print("=" * 60)
    print()
    ...
    for model_type, method_name, checkpoint_arg, pattern in model_configs:
        ...
        try:
            camera_metrics = evaluate_one_model(
                model_type, checkpoint_path, test_split_list, 
                device, use_amp, image_size, overlay_dir=args.overlay_dir
            )

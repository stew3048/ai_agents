"""
用訓練好的 DINOv2 兩層解碼頭（SkySegModel）在 5 張小樣本上評估。

規則：每次評估都要輸出 overlay（原圖 + 預測/GT 疊圖，TP=綠/FP=紅/FN=藍）。
預設 --overlay_dir 會寫入，可指定其他路徑或 --no_overlay 關閉。

用法:
  python scripts/eval_dino_linear_small.py
  python scripts/eval_dino_linear_small.py --test_list outputs/test_list_small.txt --checkpoint ... --overlay_dir outputs/dino_linear_small_overlays
"""
import os
import sys
import argparse
...
def create_overlay(image_path, pred_mask, gt_mask=None, alpha=0.5):
    """原圖 + 預測/GT 疊圖：TP=綠、FP=紅、FN=藍。pred/gt 為 numpy 0~1，與 image 同尺寸。"""
    ...
    if pred.ndim == 3:
        pred = pred[0]
    if pred.shape[:2] != (h, w):
        pred = np.array(Image.fromarray((np.clip(pred, 0, 1) * 255).astype(np.uint8)).resize((w, h), Image.NEAREST), dtype=np.float32) / 255.0
    pred_b = (pred > 0.5)
    ...
    if gt_mask is not None:
        ...
        if gt.ndim == 3:
            gt = gt[0]
        if gt.shape[:2] != (h, w):
            gt = np.array(Image.fromarray((np.clip(gt, 0, 1) * 255).astype(np.uint8)).resize((w, h), Image.NEAREST), dtype=np.float32) / 255.0
        gt_b = (gt > 0.5)
        fn_mask = gt_b & (~pred_b)
        fp_mask = pred_b & (~gt_b)
        tp_mask = gt_b & pred_b
...
def load_gt_mask(mask_path, target_h, target_w):
    ...
    t = interpolate(t, size=(target_h, target_w), mode='nearest')
    return t
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str,
                        default=os.path.join(_root, 'outputs', 'train_dino_linear_20260214_022830', 'checkpoints', 'best.pth'))
    ...
    parser.add_argument('--overlay_dir', type=str, default=None,
                        help='overlay 輸出目錄（預設：與 checkpoint 同層的 dino_linear_small_overlays）')
    parser.add_argument('--no_overlay', action='store_true', help='不輸出 overlay（不建議；評估應一律產 overlay）')
    args = parser.parse_args()

    if args.overlay_dir is None and not args.no_overlay:
        base = os.path.dirname(os.path.dirname(args.checkpoint))
        args.overlay_dir = os.path.join(base, 'dino_linear_small_overlays')
    ...
    # 載入模型
    print('載入模型: SkySegModel (DINOv2 + 兩層解碼頭)')
    ...
    for item in test_list:
        ...
        # Overlay：一律產出（與其他方法相同評估流程）
        if not args.no_overlay and args.overlay_dir:
            pred_probs = torch.sigmoid(logits).cpu().numpy().squeeze()
            if pred_probs.ndim == 3:
                pred_probs = pred_probs[0]
            pred_orig = np.array(
                Image.fromarray((np.clip(pred_probs, 0, 1) * 255).astype(np.uint8))
                .resize((w_orig, h_orig), Image.NEAREST), dtype=np.float32) / 255.0
    ...
    # 輸出
    print('=' * 60)
    print('  DINOv2 兩層解碼頭 @ 小樣本 5 張 評估結果')
    print('=' * 60)
    print()
    for r in results:

"""
訓練 DINO → DL 方案4：SegFormer-like Decoder（MLP-based）
...
輸出：
- outputs/train_dino_mlp_YYYYMMDD_HHMMSS/checkpoints/best.pth
- outputs/train_dino_mlp_YYYYMMDD_HHMMSS/training_log.csv
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(description='訓練 DINO → DL 方案4（MLP-based）')
    parser.add_argument('--device', type=str, default=None,
                        help='設備（cpu/cuda），預設自動偵測；如果設為 cpu 則強制使用 CPU')
    args = parser.parse_args()
    
    print("=" * 60)
    print("  訓練 DINO → DL 方案4：SegFormer-like Decoder（MLP-based）")
    print("=" * 60)
    print()
    ...
    output_dir = f"outputs/train_dino_mlp_{timestamp}"
